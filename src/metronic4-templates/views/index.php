...
<?php endif;?>
 <div class="row">
		<div class="col-md-12">
				...
				<div class="portlet-body" style="display: block;">
					<div class="row dataTables_extended_wrapper">
						<div class="col-md-6 col-sm-12">
							@permission('<?=$gen->authAttr?>-create')
							<a class="btn btn-sm blue-madison" href="{{ route('<?=strtolower($gen->moduleName)?>.<?=$gen->route()?>.create') }}" target="ajax-modal"><i class="fa fa-plus"></i> Tambah</a>
							@endpermission
							<?php if($export):?>
			...
			<?php
        if(in_array($field->name,\App\Generator\src\DbOracle::skippedFields()) && $field->name!='created_at') continue;
        if(($field->type == 'number')&&(preg_match("/id_|_id/",$field->name,$match))){
          $fieldName = strtolower(preg_replace("/id_|_id/","",$field->name));
          echo '<!-- <th>'.title_case($fieldName).'</th> -->';
        }
      ?>
...
$(function() {
	...
	$('.trash-ck').click(function(){
		if ($('.trash-ck').prop('checked')) {
			document.location = '{{ url("<?=strtolower($gen->moduleName)?>/<?=$gen->route()?>?status=trash") }}';
		} else {
			document.location = '{{ url("<?=strtolower($gen->moduleName)?>/<?=$gen->route()?>") }}';
		}
	});
    $('#<?=$gen->route()?>-table').DataTable({
        ...
        ajax : {
				<?php if(in_array('deleted_at',$fieldsArr)) { ?>
					...
					url:"{{ url('<?=strtolower($gen->moduleName)?>/<?=$gen->route()?>/load-data') }}?status=trash",
					@else
					url:"{{ url('<?=strtolower($gen->moduleName)?>/<?=$gen->route()?>/load-data') }}",
					@endif
				<?php }else{ ?>
					url:"{{ url('<?=strtolower($gen->moduleName)?>/<?=$gen->route()?>/load-data') }}",
				<?php } ?>
        ...
        columns: [
        	...
        	<?php $count=1;?>
			<?php foreach ( $fields as $field )  { ?>
			<?php if(in_array($field->name,\App\Generator\src\DbOracle::skippedFields()) && $field->name!='created_at') continue;
        if(($field->type == 'number')&&(preg_match("/id_|_id/",$field->name,$match))){
          $fieldName = strtolower(preg_replace("/id_|_id/","",$field->name));
          echo '//{ data: \''.$fieldName.'\', name: \''.$fieldName.'\' },';
          echo "\n";
        }
      ?>
        ...
        buttons: [
               {
                   ...
                   className: 'btn btn-outline btn-custom-toolbar',
                   action: function ( e, dt, node, config ) {
                       dt.ajax.reload();
                       // alert('Datatable reloaded!');
                   }
               },
               { extend: 'excel', className: 'btn btn-outline btn-custom-toolbar',text: '<i class="fa fa-file-excel-o"></i>',
                 exportOptions:{
                   columns:[<?php echo implode(',',range(0,$count))?>]
                 }},
